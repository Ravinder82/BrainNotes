...
Confabula is a native iOS application built with Swift and SwiftUI that allows users to create and manage specialized AI bots for specific tasks. Instead of real contacts, users interact with purpose-built bots — some created manually, others auto-generated by the Chief Bot (Captain).
...
├── ConfabulaTests/               # Unit tests
├── ConfabulaUITests/             # UI/integration tests
├── project.yml                   # Xcode project config (xcodegen)
├── Confabula.xcodeproj/          # Generated Xcode project
├── Confabula.entitlements
└── Info.plist
```
...
```bash
# Open in Xcode
open Confabula.xcodeproj

# Or build from command line
xcodebuild -project Confabula.xcodeproj \
  -scheme Confabula \
...
- **Combine** — `ObservableObject` with `@Published`/`@Published` for reactive UI updates
...
```bash
# Unit tests (no simulator required for logic tests)
xcodebuild -project Confabula.xcodeproj \
  -scheme ConfabulaTests \
  -destination 'platform=iOS Simulator,name=iPhone 16,OS=18.1' \
  test

# Run a specific test suite
xcodebuild test-without-building \
  -project Confabula.xcodeproj \
  -scheme ConfabulaTests \
  -destination 'platform=iOS Simulator,name=iPhone 16,OS=18.1' \
  -only-test ConfabulaTests.CaptainActionTests

# UI tests
xcodebuild test \
  -project Confabula.xcodeproj \
  -scheme ConfabulaUITests \
  -destination 'platform=iOS Simulator,name=iPhone 16,OS=18.1'
```
